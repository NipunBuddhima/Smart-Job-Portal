...
const parseCandidateEntry = (
  value: unknown,
  requiredFields: Array<'institution' | 'degree' | 'company' | 'title' | 'startDate'>
) => {
...
export const validateCandidateProfileUpdate = (body: CandidateProfileBody): ValidationResult<CandidateProfileUpdate> => {
  const errors: string[] = [];
  const value: CandidateProfileUpdate = {};
  ...
  if (body.socialLinks && typeof body.socialLinks === 'object') {
    const socialLinks = body.socialLinks as Record<string, unknown>;
    ...
    if (linkedin === null) errors.push('LinkedIn must be a valid URL');
    if (github === null) errors.push('GitHub must be a valid URL');
    if (portfolio === null) errors.push('Portfolio must be a valid URL');
...
export const validateEmployerProfileUpdate = (body: EmployerProfileBody): ValidationResult<EmployerProfileUpdate> => {
  const errors: string[] = [];
  const value: EmployerProfileUpdate = {};
